...
from breathm.detection import (
    auto_detect_game_info,
    cached_game_detection_is_valid,
    get_game_file_signature,
    is_botw_wua,
)
...
class BreathMLauncher(QWidget):
    ...
    def refresh_game_detection_if_needed(self) -> None:
        ...
        if cached_game_detection_is_valid(profile, game_path):
            return

        region, game_version, dlc_version = auto_detect_game_info(game_path)
    ...
    def save_game_path_from_input(self) -> None:
        ...
        if cached_game_detection_is_valid(profile, path):
            self.refresh_labels()
            return

        region, game_version, dlc_version = auto_detect_game_info(path)
    ...
    def pick_game(self) -> None:
        ...
        if not is_botw_wua(path):
            QMessageBox.critical(
                self,
                "Invalid Game",
                "This .wua does not appear to be The Legend of Zelda: Breath of the Wild.",
            )
            return
